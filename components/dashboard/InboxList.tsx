    ...
    return (
        <div className="space-y-8">
            <div>
                ...
                {pending.length === 0 && (
                    <div className="p-8 border rounded-xl border-dashed flex flex-col items-center justify-center text-slate-500 bg-slate-50/50">
                        ...
                        {pending.map((record) => (
                            ...
                            >
                                ...
                                <div className="relative">
                                    <div className="absolute top-2 left-3 -translate-y-1/2 bg-white px-1 text-[10px] font-bold text-primary-600">
                                        Sugestão da IA
                                    </div>
                                    <textarea
                                        className="w-full rounded-md border border-slate-200 bg-white pt-4 pb-2 px-3 text-sm min-h-[80px] focus:outline-none focus:ring-2 focus:ring-primary-500 focus:border-transparent disabled:opacity-50"
            ...
            {completed.length > 0 && (
                <div>
                    <h3 className="text-lg font-bold mb-4 text-slate-700">Histórico Recente</h3>
                    <div className="space-y-3 opacity-70">
                        {completed.map(record => (
                            <div key={record.id} className="border p-4 rounded-lg bg-slate-50 text-sm">
                                <div className="flex gap-2 items-center mb-1">
                                    <CheckCircle2 className="h-4 w-4 text-green-500" />
                                    <span className="text-slate-500 text-xs">Aprovado e Enviado em {new Date(record.created_at).toLocaleDateString()}</span>
                                </div>
                                <div className="text-slate-600 flex flex-col gap-1 mt-2">
                                    <p><strong className="text-slate-800">Cliente:</strong> "{record.message}"</p>
                                    <p><strong className="text-primary-600">Sua Marca:</strong> {record.ai_response}</p>
                                </div>
                            </div>
                        ))}
                    </div>
