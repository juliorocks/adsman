...
export async function generateCreativeImages(prompt: string, count: number = 4): Promise<string[]> {
    try {
        console.log(`[creatives] Generating ${count} image variations via Creative Engine (Google Imagen)...`);

        const apiKey = process.env.GEMINI_API_KEY;
        if (!apiKey) {
            console.error("[creatives] GEMINI_API_KEY not configured for image generation.");
            return [];
        }

        const ai = new GoogleGenAI({ apiKey });

        // Ensure prompt is within limits and optimal
        ...
        if (!response.generatedImages || response.generatedImages.length === 0) {
            console.error("[creatives] No images generated by the model.");
            return [];
        }

        // Map base64 bytes to data URI strings usable directly by img src
